...
export default function OnJobSupportPage() {
  ...
  const parseTwoPart = (htmlString: string) => {
    ...
    if (typeof htmlString === "object" && htmlString !== null) {
      const obj = htmlString as { part1?: string; part2?: string };
      return { part1: obj.part1 ?? "", part2: obj.part2 ?? "" };
    }
    const regex = /<span>(.*?)<\/span>/i;
    const highlightMatch = regex.exec(htmlString || "");
    ...
    async function load() {
      try {
        ...
        if (data.why_points) {
          ...
          if (Array.isArray(data.why_points) && data.why_points.length > 0) {
            // Convert array of strings to HTML with <p> tags
            const htmlContent = data.why_points
        ...
        if (data.demo_title) {
          try {
            if (typeof data.demo_title === "string") {
              setPageHeading(data.demo_title);
            } else if (typeof data.demo_title === "object") {
            ...
            if (process.env.NODE_ENV === "development") {
              console.debug("Error setting page heading:", error_);
            }
        ...
        if (Array.isArray(data.demo_points) && data.demo_points.length > 0) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const parseWhyPoints = (htmlContent: string): string[] => {
      ...
      try {
        ...
        console.error('Error parsing why points HTML:', error);
