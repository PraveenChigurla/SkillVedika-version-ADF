...
export default function ContactPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const res = await fetch(API_PROXY, { method: "GET" });
  ...
  setSaving(true);
    ...
    try {
      ...
      console.log("POST /api/contact response status:", res.status, "body:", parsed ?? text);
      ...
      if (!res.ok || res.status < 200 || res.status >= 300) {
        ...
        console.error("Full error response:", parsed);
        toast.error(msg);
        throw new Error(msg);
      }

      // Check if response indicates success
      const responseObj = parsed && typeof parsed === "object" ? parsed as Record<string, unknown> : null;
      ...
      try {
        ...
        const reloadRes = await fetch(API_PROXY, { 
          method: "GET",
        ...
        try {
          reloadBody = JSON.parse(reloadText);
        } catch {
          reloadBody = { message: reloadText };
        }
        console.log("Reloaded data:", reloadBody);
        
        // Type guard for body
        const bodyObj = reloadBody && typeof reloadBody === "object" ? (reloadBody as Record<string, unknown>) : {};
