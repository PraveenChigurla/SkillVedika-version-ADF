...
export default function HomepageDetailsPage() {
    ...
    fetch(`/api/homepage`, { credentials: "include" })
      ...
      .then((data) => {
        ...
        if (data.job_assistance_points) {
          ...
          setJob1Content(data.job_assistance_points[0]?.desc || data.job_assistance_points[0]?.content || "");

          setJob2Title(data.job_assistance_points[1]?.title || "");
          setJob2Content(data.job_assistance_points[1]?.desc || data.job_assistance_points[1]?.content || "");

          setJob3Title(data.job_assistance_points[2]?.title || "");
          setJob3Content(data.job_assistance_points[2]?.desc || data.job_assistance_points[2]?.content || "");

          setJob4Title(data.job_assistance_points[3]?.title || "");
          setJob4Content(data.job_assistance_points[3]?.desc || data.job_assistance_points[3]?.content || "");

          setJob5Title(data.job_assistance_points[4]?.title || "");
          setJob5Content(data.job_assistance_points[4]?.desc || data.job_assistance_points[4]?.content || "");

          setJob6Title(data.job_assistance_points[5]?.title || "");
          setJob6Content(data.job_assistance_points[5]?.desc || data.job_assistance_points[5]?.content || "");
        }
  ...
  async function saveHomepage() {
    ...
    try {
      ...
      const url = isNewRecord ? `/api/homepage` : `/api/homepage/${homepageId}`;
      ...
      if (res.ok) {
        if (isNewRecord) {
          ...
          setHomepageId(result.data?.id || null);
        } else {
        ...
        toast.error(result.message || "Failed to save homepage.", {
          duration: 4000,
          position: "top-right",
        });
      }
              ...
              <div className="border border-gray-200 rounded-xl p-5 bg-gray-50">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  {exploreTabs.map((tab, i) => {
                    ...
                    return (
                    <div key={`explore-tab-${i}-${tab}`} className="space-y-2">
