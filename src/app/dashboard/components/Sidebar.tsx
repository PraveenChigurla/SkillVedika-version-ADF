...
export default function Sidebar({ isOpen }: Readonly<SidebarProps>) {
  ...
  const getMenuItemClasses = (isActive: boolean): string => {
    const baseClasses = "w-full flex items-center justify-between px-4 py-3 rounded-lg cursor-pointer transition-all duration-200";
    if (isActive) {
        ...
        <button
          onClick={() => {
            if (globalThis.document !== undefined) {
              globalThis.document.dispatchEvent(new Event("logout"));
            }
          }}
