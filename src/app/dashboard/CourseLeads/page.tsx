...
import { useCallback, useEffect, useMemo, useState, useRef, ReactNode } from "react";
...
export default function CourseLeads() {
  ...
  const fetchLeads = useCallback(async () => {
    try {
      ...
      console.log("[CourseLeads] 🚀 fetchLeads called with currentPage:", currentPage, "pageSize:", pageSize);
      ...
      console.log("[CourseLeads] 🔍 Fetching leads with params:", JSON.stringify(params, null, 2));
      ...
      console.debug("[CourseLeads] fetched leads sample:", normalized.slice(0, 3));
      ...
      if (normalized.length > 0) {
        ...
        console.log("[CourseLeads] 📊 Data verification:", {
          ...
          firstLeadId,
          lastLeadId,
          expectedRange: `Leads ${(currentPage - 1) * pageSize + 1} to ${currentPage * pageSize}`,
        });
      }
      ...
      console.log("[CourseLeads] 📝 Updated lastFetchParamsRef:", {
        ...
        previousKey: lastFetchParamsRef.current ? lastFetchParamsRef.current.substring(0, 100) + "..." : "(empty)",
      });
      console.log('--------------------------------');
  ...
  useEffect(() => {
    ...
    const lastFetch = lastFetchParamsRef.current ? JSON.parse(lastFetchParamsRef.current) : null;
    const pageChanged = lastFetch && lastFetch.page !== currentPage;
    
    if (!pageChanged && lastFetchParamsRef.current === fetchKey) {
      console.log("[CourseLeads] ⏭️ Skipping duplicate fetch with same params", {
    ...
    console.log("[CourseLeads] 🔑 Fetch key comparison:", {
      currentKey: fetchKey.substring(0, 100) + "...",
      lastKey: lastFetchParamsRef.current ? lastFetchParamsRef.current.substring(0, 100) + "..." : "(empty)",
      currentPage,
      pageChanged,
      willFetch: true,
    });
  ...
  const fetchLeadDetails = useCallback(async (leadId: number) => {
    try {
      const res = await axios.get(`/leads/${leadId}`, {
        headers: { Accept: "application/json" },
      });
  ...
  const updateStatus = useCallback(
    async (status: string) => {
      ...
      try {
        ...
        // Refresh leads to ensure we have the latest data from database
        void fetchLeads();
      } catch (err: any) {
    ...
    [selectedLead, ensureCsrf, authCfg, router, fetchLeads]
  );
  ...
  const downloadExcel = useCallback(async () => {
    try {
      ...
      const res = await axios.get("/leads/export", {
        params: exportParams,
        headers: { Accept: "application/json" },
      });
      ...
      if (!responseData?.success) {
        toast.error("Failed to export leads: " + (responseData?.message ?? "Unknown error"));
        return;
      }
      ...
      const normalizedLeads = allLeads.map((r: any) => {
        const coursesArr = extractCoursesArray(r, courseMap);
        const joinedCourse = coursesArr.length > 0 
          ? coursesArr.join(", ") 
      ...
      if (courseFilter) {
        const courseName = allCourses.find(c => c.id === courseFilter)?.title || courseFilter;
      ...
      toast.error("Failed to export leads: " + (err?.response?.data?.message || err?.message || "Unknown error"));
    }
  }, [
    debouncedSearch,
    sortBy,
  ...
  /* ---------------------------
     Pagination helper
  --------------------------- */
  function getPageRange(current: number, last: number, delta = 2): (number | "gap")[] {
    if (last <= 0) return [];
    if (last === 1) return [1];
  ...
  return (
    <div className="min-h-screen bg-[#f9fafb] p-6">
      <section className="mx-auto " aria-labelledby="page-title">
        {/* Header Section */}
        <div className="mb-8">
            ...
            <div className="flex flex-col sm:flex-row gap-3 items-stretch w-full sm:w-auto">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  disabled={loading}
                  className="px-5 py-3 bg-white border border-gray-200 rounded-xl shadow-sm text-sm font-medium text-gray-700 hover:shadow-md hover:bg-gray-50 transition-all duration-200 flex items-center gap-2 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 xl:grid-cols-7 gap-4 items-end">
              <div className="flex flex-col">
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    console.log("[CourseLeads] Status filter changed:", newValue);
              ...
              <div className="flex flex-col">
                ...
                <select
                  ...
                  onChange={(e) => {
                    const newValue = e.target.value;
                    console.log("[CourseLeads] Course filter changed:", newValue);
              ...
              <div className="md:col-span-2 lg:col-span-2 xl:col-span-2 flex items-end justify-end gap-3">
                <div className="flex flex-col gap-2">
                  <label htmlFor="sortBy" className="text-xs font-semibold text-gray-600 uppercase tracking-wide whitespace-nowrap">
                    Sort
                  </label>
                  <select
                    id="sortBy"
                ...
                <div className="flex items-center gap-2">
                  <label htmlFor="pageSize" className="text-xs font-semibold text-gray-600 uppercase tracking-wide">
                    Per page
                  </label>
                  <select
                    id="pageSize"
                          ...
                          <td className="py-2 px-3 max-w-[200px]">
                            {(() => {
                              ...
                              if (coursesToShow.length === 1) {
                                return (
                                  <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-gray-100 text-gray-700 border border-gray-200" title={coursesToShow[0]}>
                                    {coursesToShow[0].length > 25 ? `${coursesToShow[0].substring(0, 25)}...` : coursesToShow[0]}
                                  </span>
                                );
                              }
                              ...
                              return (
                                <div className="flex flex-wrap gap-1">
                                  <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-gray-100 text-gray-700 border border-gray-200" title={firstCourse}>
                                    {firstCourse.length > 20 ? `${firstCourse.substring(0, 20)}...` : firstCourse}
                                  </span>
                          ...
                          <td className="py-2 px-3 text-center">
                            ...
                            >
                              ...
                              <button
                                ...
                                className="inline-flex items-center justify-center w-7 h-7 rounded-lg bg-red-50 text-red-600 hover:bg-red-600 hover:text-white transition-all duration-200"
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="text-xs text-slate-600 mt-2">
                        {(() => {
                          ...
                          if (coursesToShow.length === 0) return <span className="text-slate-400">No course</span>;
                          if (coursesToShow.length === 1) {
                            return <span className="truncate block" title={coursesToShow[0]}>{coursesToShow[0]}</span>;
                          }
                          return (
                            <div>
                              <span className="truncate block" title={coursesToShow[0]}>{coursesToShow[0]}</span>
                              <span className="text-slate-400">+{coursesToShow.length - 1} more</span>
                            </div>
                          );
                        })()}
                      </div>
              ...
              {totalLeads > 0 ? (
                <>
                  ...
                  <span className="ml-2 text-gray-500">
                    ({leads.length} {leads.length === 1 ? 'lead' : 'leads'} on this page of {totalLeads} total)
                  </span>
                </>
              ) : (
            ...
            <div className="flex items-center gap-2 text-sm">
              <button
                ...
                onClick={(e) => {
                  ...
                  console.log("[CourseLeads] Previous page clicked, changing from", currentPage, "to", newPage);
                  ...
                  return (
                    <button
                      ...
                      onClick={(e) => {
                        ...
                        console.log("[CourseLeads] Page number clicked, changing from", currentPage, "to", pageNum);
                        // Clear fetch cache to ensure fresh data for new page
                        lastFetchParamsRef.current = "";
              ...
              <button
                ...
                onClick={(e) => {
                  ...
                  if (newPage === currentPage) return;
                  console.log("[CourseLeads] Next page clicked, changing from", currentPage, "to", newPage);
                  // Clear fetch cache to ensure fresh data for new page
                  lastFetchParamsRef.current = "";
                  setCurrentPage(newPage);
                }}
      ...
      {isOpen && selectedLead ? (
            ...
            <div className="p-6 space-y-6 overflow-y-auto flex-1 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
              ...
              <div className="bg-gray-50 rounded-xl p-5 border border-gray-100">
                <h4 className="text-sm font-semibold text-gray-700 mb-4 uppercase tracking-wide">
                  Courses{" "}
                  {(() => {
                    if (selectedLead.courses && selectedLead.courses.length > 1) {
                ...
                <div className="flex flex-wrap gap-2">
                  {(() => {
                    ...
                    if (coursesToDisplay.length === 0) {
                      return (
                        <span className="text-sm text-gray-400 italic">No courses specified</span>
                      );
                    }
        ...
        <div
          ...
          aria-label="Close modal"
        >
          <div className="bg-white rounded-2xl p-8 shadow-2xl border border-gray-100" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center gap-3">
              <div className="animate-spin rounded-full h-6 w-6 border-b-2 border-[#1A3F66]" aria-hidden="true"></div>
              <span className="text-gray-700 font-medium">Loading lead details...</span>
            </div>
          </div>
        </div>
      ) : null}
